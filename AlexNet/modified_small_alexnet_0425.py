...
def alexnet_model(img_shape=(50, 50, 3), n_classes=2, l2_reg=0.,
    ...
    # Layer 1
    alexnet.add(Conv2D(12, (11, 11), input_shape=img_shape,
    ...
    alexnet.add(Conv2D(48, (3, 3), padding='same'))
    alexnet.add(BatchNormalization())
    alexnet.add(Activation('relu'))
...
X_red, y_red = procData(all_data, 0,200000)
X_red, y_red = np.asarray(X_red), np.asarray(y_red)

X_test, y_test = procData(all_data, 210000, 270000)
...
model.save("modified_alexnet_trainedmodel_GPU.h5") # saving the model 
